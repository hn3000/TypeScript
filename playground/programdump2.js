...
var structureDumper = function(context, n, after) {
  ...
  if (n.kind == ts.SyntaxKind.ClassDeclaration) {
    ...
    descend = true;
  } else if (n.kind == ts.SyntaxKind.FunctionBlock) {
    if (!after) {
      console.log('    +'+ ts.SyntaxKind[n.kind]);
    }
    descend = true;
  } else if (n.kind == ts.SyntaxKind.ReturnStatement) {
    if (!after) {
      console.log('    +'+ ts.SyntaxKind[n.kind]);
    }
    descend = true;
  } else if (n.kind == ts.SyntaxKind.ParenExpression) {
    if (!after) {
      console.log('    +'+ ts.SyntaxKind[n.kind]);
    }
    descend = true;
  } else if (n.kind == ts.SyntaxKind.BinaryExpression) {
    if (!after) {
      console.log('    '+ n.operator  + '   ' + ts.SyntaxKind[n.kind]);
    }
    descend = true;
  } else if (n.kind == ts.SyntaxKind.PropertyAccess) {
    if (!after) {
      console.log('     '+ n.left.text + '.' + n.right.text  + '   ' + ts.SyntaxKind[n.kind]);
    }
    descend = true;
